...
/*
    ...
    Easily create fancy text in spigot.
    For example text that displays text when you hover over it or runs a command when you click it.
 
    To create a Text object use the method Text.of(String) or the constructor new Text(String)
 
    The input string should have color codes ALREADY TRANSLATED.
 
    To add hover text or run a command when clicked you use brackets.
    Brackets follow the format: {type=value}TEXT{/}
    To use multiple effects on a text just chain the beginning brackets, the single ending bracket counts for all of them.
    Example: {type1=value1}{type2=value2}{type3=value3}TEXT{/}
    If you want to color the text in the bracket make sure you do it INSIDE the bracket or it won't be colored.
 
    Example to add hover text:
    "§7You have been promoted to {hover=§eLeader is the highest rank on the server}§4LEADER{/}§7."
 
    Example to make player run command when text clicked:
    "§7Click {cmd=/sg join sg-1}§eHERE{/} §7to go to SG."
 
    Example with multiple effects on text:
    "§7You muted {hover=§bClick to unmute.}{cmd=/unmute Notch}§aNotch{/}§7.
