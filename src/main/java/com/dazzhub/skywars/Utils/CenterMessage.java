...
public class CenterMessage {
    ...
    public enum Chat {
        ...
        private char character; private int length;
    ...
    public static String centerMessage(String replace) {
        ...
        for (int length = (charArray = replace.toCharArray()).length, i = 0; i < length; ++i) {
            ...
            if (c == '§') {
                ...
                Chat fontInfo = Chat.getDefaultFontInfo(c);
                n += (b ? fontInfo.getBoldLength() : fontInfo.getLength());
                ++n;
            }
        }
        int n3 = 154 - n / 2;
        int n4 = Chat.SPACE.getLength() + 1;
        int j = 0;
